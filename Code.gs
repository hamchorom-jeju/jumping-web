...
function searchMembersByDigits(payload) {
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      // [출석 앱 공식] 모든 회원 + 뒷 4자리 엄격 매칭 (마감/기간만료 등 포함)
      if (status !== "" && phoneOnlyDigits.slice(-4) === digits) {
...
function getCompiledMemberRegistry(ss) {
  ...
  for (var j = 0; j < keys.length; j++) {
    ...
    var allExpired = m.passes.every(function(p) { return p.status !== "진행중" && p.status !== "진행 중"; });
...
function generateAttendanceReportAiComment(name, stats, prevSms, remain, expire, startDateStr, endDateStr) {
  try {
    ...
    var systemInstruction = "당신은 제주 노형점핑클럽의 따뜻하고 쾌활하며 전문적인 웰니스 코치입니다. " +
                            ...
                            "특히 프롬프트 예시에 있는 상투적인 미사여구나 문장을 그대로 복사해 쓰지 마십시오. 대상 기간의 실제 계절(예: 6~8월 여름철은 청량/활기찬 분위기, 12~2월 겨울철은 포근한 분위기 등)에 맞는 알맞은 날씨 인사와 회원의 실제 출석 횟수(특히 0회 출석 회원에게 '매일 소중한 시간을 개척한다'는 어울리지 않는 칭찬을 하는 것 절대 금지)에 부합하는 현실적이고 자연스러운 안부로 다채롭게 작성해야 합니다. " +
    ...
    var prompt = "■ 회원 및 출석 정보\n" +
                 ...
                 "1. 편지 구조 정의:\n" +
                 "   - [첫인사]: 회원님의 이름을 다정하고 따뜻하게 부르며 시작하는 친근한 첫인사. 오늘이 몇 월 며칠이든 관계없이 편지 내용 상에서는 반드시 " + periodLabel + "의 건강 여정 정산 소식임을 명시하십시오. (예: '♥지은 회원님, 활기찬 " + greetingLabel + " 🌟 소중한 웰니스 성적표를 들고 코치가 찾아왔습니다!')\n" +
                 "   - [본문 (출석 분석)]: " + analysisLabel + " 남은 횟수, 그리고 이전 히스토리 대비 증감을 분석하여 칭찬/공감/우려를 전문적이면서 다정하게 언급\n" +
                 "   - [끝인사]: " + nextMonthLabel + " 힘차게 건강 관리를 이어나갈 수 있도록 따뜻하게 격려하고 클럽에서 반갑게 만나자는 다정한 끝인사. (예시: 출석이 좋은 회원에게는 '바쁘신 일상 속에서도 자신을 위한 소중한 시간을 개척해 나가시는 모습이 멋집니다. " + nextMonthLabel + " 클럽에서 뵙겠습니다. ❤️'와 같이 칭찬을, 출석이 낮은(특히 0~2회) 회원에게는 과도한 성실함 칭찬을 절대 하지 말고 '바쁜 일정이 정리되시는 대로 " + nextMonthLabel + " 클럽에서 반가운 얼굴로 꼭 뵈어요! ❤️'와 같이 따뜻한 복귀 독려를 하십시오.)\n" +
                 "2. 출석 빈도별 반응 룰:\n" +
                 ...
                 "3. 지난 정산 이력과의 1:1 대조 분석 및 표현 다양성 확보:\n" +
                 "   - 지난 문자 기록이 있다면 꼭 이전 출석 횟수와 비교하여 증가/감소의 변화 양상을 극찬하거나 공감하며 안부를 물으십시오.\n" +
                 "   - 중복/반복 표현 방지: 매번 정산 문자를 받는 회원이 식상함을 느끼지 않도록, '지난 정산 문자 기록'에서 사용된 독특한 단어, 핵심 어구, 은유적 표현(예: '웰니스 성적표', '숨고르기', '건강 여정', '성적표' 등)을 절대로 재사용하지 마십시오. 문장 구조와 흐름을 완전히 새롭게 설계하여 매번 새로운 웰니스 편지를 받는 느낌이 나도록 작성해야 합니다.\n" +
                 "4. 출력 형식:\n" +
