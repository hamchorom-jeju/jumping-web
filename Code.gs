...
/**
 * 🤖 [v66.0] 제미나이를 통한 개별 회원 맞춤형 한 달 출석 평가 코멘트 생성기 (초고속 응답 최적화)
 */
function generateAttendanceReportAiComment(name, stats, prevSms, remain, expire, startDateStr, endDateStr) {
  try {
    var systemInstruction = "제주 노형점핑클럽의 친근한 웰니스 코치로서 회원의 출석률에 대해 격려, 걱정, 칭찬 코멘트를 작성하십시오. " +
                            "친근하게 존댓말로 70자~130자 내외로 매우 직관적이고 빠르게 작성하고, 긍정적인 이모지 한두 개를 섞으십시오. 오직 큰따옴표 안에 들어갈 문장만 짧게 쓰십시오.";
    
    var prompt = "■ 회원 상황\n" +
                 "- 이름: " + name + "\n" +
                 "- 기간 출석: 총 " + stats.total + "회 (점핑 " + stats.jumping + "회 / 테라피 " + stats.therapy + "회)\n" +
                 "- 잔여 횟수: " + remain + "회, 만료일: " + expire + "\n" +
                 "- 지난 정산 문자 기록: " + (prevSms ? prevSms : "없음") + "\n\n" +
                 "■ 작성 핵심 수칙 (필수):\n" +
                 "1. 출석이 0~4회로 저조하면 남은 " + remain + "회가 만료일(" + expire + ")까지 소멸될까 걱정하는 코치의 진심어린 염려 멘트 필수 주입.\n" +
                 "2. 출석이 5회 이상 활발하면 꾸준함에 대한 찬사와 활기찬 격려 주입.\n" +
                 "3. '지난 정산 내용'이 있다면 지난 횟수와 간결히 대조하여 '지난번보다 출석이 늘어났다/줄어들었다'는 성취를 짧게 1문장 언급할 것.\n" +
                 "4. 구구절절 긴 서론은 생략하고 2~3문장의 콤팩트한 평가 본문만 즉시 반환하십시오.";
