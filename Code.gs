...
/**
 * [관리자 전용] 일일 업무일지 제출 및 데이터 저장 (중복 날짜 시 스마트 덮어쓰기 지원)
 */
function submitWorkLog(data) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = ss.getSheetByName("업무일지") || ss.insertSheet("업무일지");
    
    if (sheet.getLastRow() === 0) {
      var headers = ["날짜", "작성자", "점핑프로그램", "근력프로그램", "특이사항", "시설고장/파손", "출석통계"];
      sheet.getRange(1, 1, 1, headers.length).setValues([headers]).setFontWeight("bold").setBackground("#f1f3f5");
    }
    
    var dateVal = data.date;
    var author = data.author;
    var jumpingList = data.jumpingList;
    var muscleList = data.muscleList;
    ...
    for (var i = 1; i < sheetData.length; i++) {
      var rowDate = "";
      if (sheetData[i][0] instanceof Date) {
        rowDate = Utilities.formatDate(sheetData[i][0], Session.getScriptTimeZone(), "yyyy-MM-dd");
      } else {
        rowDate = String(sheetData[i][0]).split("T")[0].trim();
      }
      if (rowDate === dateVal) {
        targetRow = i + 1;
        break;
    ...
    var rowValues = [dateVal, author, jumpingList, muscleList, remarks, issues, statsJson];
    if (targetRow > -1) {
    ...
    return { success: true, message: "일지가 안전하게 저장되었습니다!" };
  } catch (e) {
    return { error: "업무일지 제출 실패: " + e.toString() };
  }
}

/**
 * [관리자 전용] 업무일지 기록 전체 역사 목록 조회 (최신순 역순 정렬)
 */
function getWorkLogHistory() {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = ss.getSheetByName("업무일지");
    ...
    for (var i = data.length - 1; i >= 1; i--) {
      ...
      var stats = {};
      try {
        var statsStr = String(data[i][6] || "").trim();
        if (statsStr) {
          stats = JSON.parse(statsStr);
        }
      } catch(jsonErr) {
        Logger.log("출석 통계 JSON 파싱 에러: " + jsonErr.toString());
      }
      
      logs.push({
        date: dateStr,
        author: String(data[i][1] || "").trim(),
        jumpingList: String(data[i][2] || "").trim(),
        muscleList: String(data[i][3] || "").trim(),
        remarks: String(data[i][4] || "").trim(),
        issues: String(data[i][5] || "").trim(),
        stats: stats
      });
    }
    ...
    return { error: "업무일지 기록 조회 실패: " + e.toString() };
  }
}
